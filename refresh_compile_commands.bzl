...
def refresh_compile_commands(
    ...
    script_name = name + ".py"
    _expand_template(name = script_name, labels_to_flags = targets, exclude_headers = exclude_headers, exclude_external_sources = exclude_external_sources, trust_bazel_dep_files = trust_bazel_dep_files, **kwargs)
...
_expand_template = rule(
    attrs = {
        ...
        "trust_bazel_dep_files": attr.bool(default = False),  # Reuse Bazel's `.d` files via mtime alone, skipping the action-cache guard that's unavailable on Bazel >= 9. See https://github.com/helly25/bazel-compile-commands-extractor/issues/23
