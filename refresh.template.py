...
def _get_files(compile_action):
    ...
    # Skip standalone header parsing actions (-xc++-header -fsyntax-only).
    # These are generated by the parse_headers feature (e.g. from abseil-cpp's BUILD files)
    # and run on the exec platform. clangd doesn't need them; it will pick up the header
    # via the .cc files that include it.
